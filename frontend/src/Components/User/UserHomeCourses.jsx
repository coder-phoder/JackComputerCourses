import { motion } from 'framer-motion'
import { ArrowRight, ChevronRight, Layers3, PlayCircle } from 'lucide-react'
import { Link } from 'react-router-dom'
import { getCourseAccessDisplay } from '../../utils/courseAccess'
import { fadeUp, staggerParent } from '../../utils/motion'

// A list, not a wall of cards: one line per course is what makes opening the right one
// quick. The courses page keeps the full catalogue, with the search and filters a long
// list needs.
const HOME_COURSE_LIMIT = 8

const getCount = (value) => {
  const count = Number(value)

  return Number.isFinite(count) && count > 0 ? count : 0
}

const CourseRow = ({ course }) => {
  const access = getCourseAccessDisplay(course)
  const meta = [course.category, course.level].filter(Boolean).join(' · ')

  return (
    <motion.a
      variants={fadeUp}
      href={`/user/courses/${course._id}/player`}
      target="_blank"
      rel="noreferrer"
      className="group flex items-center gap-4 px-4 py-3.5 transition hover:bg-blue-50/60 focus:outline-none focus-visible:bg-blue-50/60 dark:hover:bg-blue-950/20 dark:focus-visible:bg-blue-950/20 sm:px-5"
    >
      <span className="relative hidden h-12 w-20 shrink-0 overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800 sm:block">
        {course.thumbnailUrl ? (
          <img
            src={course.thumbnailUrl}
            alt=""
            className="h-full w-full object-cover transition duration-300 group-hover:scale-105"
          />
        ) : (
          <span className="flex h-full w-full items-center justify-center bg-blue-50 text-lg font-black text-blue-300 dark:bg-blue-950/40 dark:text-blue-700">
            J
          </span>
        )}
        <span className="absolute inset-0 flex items-center justify-center bg-slate-950/45 opacity-0 transition group-hover:opacity-100">
          <PlayCircle className="h-6 w-6 text-white" />
        </span>
      </span>

      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-bold text-slate-900 group-hover:text-blue-700 dark:text-slate-100 dark:group-hover:text-blue-300">
          {course.title}
        </span>
        <span className="mt-0.5 block truncate text-xs font-semibold text-slate-500 dark:text-slate-400">
          {meta || 'Course'}
        </span>
      </span>

      <span className="hidden shrink-0 items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 md:inline-flex">
        <Layers3 className="h-3.5 w-3.5" />
        {getCount(course.chapterCount)} chapters · {getCount(course.videoCount)} videos
      </span>

      <span className="hidden shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300 lg:inline-block">
        {access.value}
      </span>

      <ChevronRight className="h-4 w-4 shrink-0 text-slate-400 transition-transform group-hover:translate-x-1 group-hover:text-blue-600 dark:group-hover:text-blue-400" />
    </motion.a>
  )
}

const UserHomeCourses = ({ courses, loading }) => {
  const visibleCourses = courses.slice(0, HOME_COURSE_LIMIT)

  return (
    <motion.section
      variants={fadeUp}
      className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"
    >
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-4 dark:border-slate-800 sm:px-5">
        <div>
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">Your courses</h2>
          <p className="mt-0.5 text-xs font-semibold text-slate-500 dark:text-slate-400">
            {loading
              ? 'Checking what is open to you...'
              : `${courses.length} ${courses.length === 1 ? 'course is' : 'courses are'} open to you`}
          </p>
        </div>

        <Link
          to="/user/courses"
          className="group inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-blue-700 transition hover:border-blue-300 hover:bg-blue-50 dark:border-slate-700 dark:bg-slate-900 dark:text-blue-300 dark:hover:border-blue-700 dark:hover:bg-blue-950/40"
        >
          {courses.length > HOME_COURSE_LIMIT ? `View all ${courses.length}` : 'Search and filter'}
          <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
        </Link>
      </header>

      {loading ? (
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {[0, 1, 2, 3].map((placeholder) => (
            <div key={placeholder} className="flex items-center gap-4 px-5 py-3.5">
              <span className="hidden h-12 w-20 shrink-0 animate-pulse rounded-lg bg-slate-100 dark:bg-slate-800 sm:block" />
              <span className="h-4 flex-1 animate-pulse rounded bg-slate-100 dark:bg-slate-800" />
            </div>
          ))}
        </div>
      ) : visibleCourses.length ? (
        <motion.div
          variants={staggerParent(0.05, 0.06)}
          className="divide-y divide-slate-200 dark:divide-slate-800"
        >
          {visibleCourses.map((course) => (
            <CourseRow key={course._id} course={course} />
          ))}
        </motion.div>
      ) : (
        <div className="px-6 py-12 text-center">
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">No courses yet</h3>
          <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-600 dark:text-slate-300">
            Courses appear here as soon as they are published or assigned to your number.
          </p>
        </div>
      )}
    </motion.section>
  )
}

export default UserHomeCourses
